...
void gyroRead(uint8_t imu_index)
{
...
void accRead(uint8_t imu_index) {
  gyro_acc.readAcc();
...
void magRead(uint8_t imu_index) {
  mag.read();
...
{
  digitalWrite(fingers[imuId -1], LOW);
  digitalWrite(fingers[imuId], HIGH);
  //need wire transmition??
