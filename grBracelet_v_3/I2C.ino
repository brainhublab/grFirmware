...
void Read_Accel(uint8_t i) {
  ...
  if (i == 5)
  {
    ...
    SENSORS[i].accel_y = SENSOR_SIGN[4] * (SENSORS[i].A_AN[1]);
    SENSORS[i].accel_z = SENSOR_SIGN[5] * (SENSORS[i].A_AN[2]);
  }
  else
  {
    SENSORS[i].accel_x = FINGER_SENSOR_SIGN[3] * (SENSORS[i].A_AN[0]);
    SENSORS[i].accel_y = FINGER_SENSOR_SIGN[4] * (SENSORS[i].A_AN[1]);
    SENSORS[i].accel_z = FINGER_SENSOR_SIGN[5] * (SENSORS[i].A_AN[2]);
  }
}
